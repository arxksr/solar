...
export function StructureSlide({ onBack = () => {}, onNext = () => {} }) {
  ...
  const CITIES = [
    { name: 'Erbil',         lon: 44.0092, lat: 36.1911, dotRef: erbilDotRef },
    { name: 'Duhok',         lon: 42.9888, lat: 36.8665, dotRef: duhokDotRef },
    { name: 'Sulaymaniyah',  lon: 45.4338, lat: 35.5617, dotRef: sulayDotRef },
  ];
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
        <div>
          ...
          <p style={{ margin: '3px 0 0', fontSize: 9, fontWeight: 700, color: '#3b82f6', letterSpacing: '3px' }}>
            POLITICAL & ADMINISTRATIVE STRUCTURE · KURDISTAN REGION
          </p>
        </div>

        <button
          onClick={onNext}
          style={{ background: '#0f172a', color: '#fff', border: 'none', cursor: 'pointer', padding: '10px 18px', borderRadius: 10, fontSize: 10, fontWeight: 900, letterSpacing: '1.5px' }}
        >
          ECONOMIC OVERVIEW →
        </button>
      </div>
      ...
      <div style={{ flex: 1, position: 'relative', minHeight: '500px', paddingBottom: 20, boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
        ...
        <Map
          initialViewState={{ longitude: 44.2, latitude: 36.0, zoom: 6.5 }}
        ...
        >
          <Source id="kurdistan-boundary" type="geojson" data={`${BASE_PATH}/sites/Kurdistan Region-Governorates.geojson`}>
            <Layer id="boundary-fill" type="fill" paint={{ 'fill-color': '#3b82f6', 'fill-opacity': 0.11 }} />
            <Layer id="boundary-line" type="line" paint={{ 'line-color': '#1e40af', 'line-width': 2 }} />
          </Source>

          {CITIES.map(city => (
            <Marker key={city.name} longitude={city.lon} latitude={city.lat} anchor="bottom">
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <div style={{ background: '#0f172a', color: '#fff', padding: '3px 8px', borderRadius: 5, fontSize: 9.5, fontWeight: 800, letterSpacing: '0.4px', whiteSpace: 'nowrap' }}>
                  {city.name}
                </div>
                <div
                  ref={city.dotRef}
                  style={{ width: 10, height: 10, background: '#2563eb', borderRadius: '50%', border: '2px solid white', boxShadow: '0 2px 8px rgba(0,0,0,0.25)', marginTop: 3 }}
                />
              </div>
            </Marker>
          ))}
        ...
        <div style={{ position: 'absolute', top: 250, left: 10, right: 10, display: 'flex', justifyContent: 'space-between', gap: 20, pointerEvents: 'none' }}>
          ...
          <div style={{ marginLeft: 60, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 28, pointerEvents: 'auto' }}>
          ...
          <div style={{ marginRight: 60, marginLeft: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 28, pointerEvents: 'auto' }}>
